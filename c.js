...
// ===== 画面遷移データ単体 =====
if (Array.isArray(raw.screenData)) {
if (!confirm(‘現在のデータをインポートしたデータで上書きします。よろしいですか？’)) return;
localStorage.setItem(‘screenFlowData’, JSON.stringify(raw.screenData));
location.reload();
return;
...
subAcc('subDocs_gyoumu', '業務資料', [
  { name: '📂 業務資料フォルダを開く', url: 'file:///C:/SharedDocs/' },
...
subAcc('subDocs_oubai', '応対品質', [
  { name: '📂 応対品質フォルダを開く',                          url: 'file:///C:/SharedDocs/Quality/' },
...
if (mw) mw.style.display = sel.value === ‘**manual**’ ? ‘block’ : ‘none’;
saveHearingState();
...
‘<option value=”**manual**”’ + (dv === ‘**manual**’ ? ’ selected’ : ‘’) + ‘>その他（手入力）</option>’ +
‘</select>’ +
...
return ’<label class="hr-check-label"><input id=“hearingCb*’ + id + ‘” type=“checkbox” ’ + (checked ? ‘checked’ : ‘’) + ’ onchange=“onHearingCheckChange('’ + id + ‘')”>’ + ’ ’ + label + ‘</label>’;
}
...
if (s.authCodeIssue === ‘メール受信なし’) {
var domain = s.mailDomain === ‘**manual**’ ? s.mailDomainManual : s.mailDomain;
